def calFund():
    ...
    ISQ_reader = ISQ_reader.replace('total', '2000-01-01', regex=True)
    with open(r'Varians\Cash Flow Quarterly.csv') as CFQ:
        CFQ_reader = pd.read_csv(CFQ)
    CFQ_reader = CFQ_reader.replace('total', '2000-01-01', regex=True)
    with open(r'Varians\Balance Sheet Quarterly.csv') as BSQ:
        BSQ_reader = pd.read_csv(BSQ)    
    BSQ_reader = BSQ_reader.replace('total', '2000-01-01', regex=True)
